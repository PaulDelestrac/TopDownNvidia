...
class TopDown:
    ...
    def __init__(self):
        ...
        parser: argparse.ArgumentParse = (
            argparse.ArgumentParser(  # prog='[/path/to/PROGRAM]',
                ...
                usage="%(prog)s [OPTIONS] -f [PROGRAM] -l [NUM]",
            )
        )  # exit_on_error=False)
    ...
    def __add_level_argument(self, group: argparse._ArgumentGroup):
        ...
        group.add_argument(
            TopDownParameters.C_LEVEL_ARGUMENT_SHORT_OPTION,
            TopDownParameters.C_LEVEL_ARGUMENT_LONG_OPTION,
            required=True,
            ...
            nargs=1,
            choices=range(
                TopDownParameters.C_MIN_LEVEL_EXECUTION,
                TopDownParameters.C_MAX_LEVEL_EXECUTION + 1,
            ),  # range [1,3], produces error,
            metavar="[NUM]",
            dest="level",
        )
    ...
    def __add_arguments(self, parser: argparse.ArgumentParser):
        ...
        requiredGroup: argparse._ArgumentGroup = parser.add_argument_group(
            "Required arguments"
        )
    ...
    def __intro_message(self):
        ...
        printer.print_desplazed_underlined_str(
            message=message,
            output_file=self.output_file(),
            delete_content_file=self.delete_output_file_content(),
        )
        print()
        message = (
            "\n\nWelcome to the "
            + sys.argv[0]
            + " program where you can check the bottlenecks of your\n"
        ...
        printer.print_max_line_length_message(
            message,
            TopDownParameters.C_NUM_MAX_CHARACTERS_PER_LINE,
            self.output_file(),
            False,
        )
        print()
        ...
        message = (
            ...
            + "\n"
            + "- Verbose:                          "
            + str(self.show_verbose())
            + "\n"
            + "- Delete output's file content:     "
            + str(self.delete_output_file_content())
            + "\n"
            + "- Output Graph File:                "
            + str(self.output_graph_file())
            + "\n"
            + "- Show Graph:                       "
            + str(self.show_graph())
            + "\n"
            + "- Input Scan File:                  "
            + str(self.input_scan_file())
            + "\n"
            + "- Output Scan File:                 "
            + str(self.output_scan_file())
        )
        ...
        if not execute_with_nvprof:
            if show_events:
                show_events_with_nsight = (
                    ". You have introduce the option\n"
        ...
        message += (
            "\n- Show Metrics:                     "
            + str(self.show_metrics())
            + "\n"
            + "- Show Events:                      "
            + str(self.show_events())
            + show_events_with_nsight
            + "- Show All Measurements:            "
            + str(self.show_all_measures())
        )

        printer.print_msg_box(
            msg=message,
            indent=1,
    ...
    def __show_level_one_results(self, level_execution: LevelOne):
        ...
        stalls_front_message: str = "{:<20} {:<6}".format(
            "STALLS, on the total (%): ",
            str(
                round(
                    level_execution.front_end_stall(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        )
        stalls_back_message: str = "{:<20} {:<6}".format(
            "STALLS, on the total (%): ",
            str(
                round(
                    level_execution.back_end_stall(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        )
        ipc_degradation_divergence_message: str = "{:<20} {:<6}".format(
            "IPC DEGRADATION (%): ",
            str(
                round(
                    level_execution.divergence_percentage_ipc_degradation(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        )
        ipc_retire_message: str = "{:<21} {:<4}".format(
            "PERFORMANCE IPC (%):",
            str(
                round(
                    level_execution.retire_ipc_percentage(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        )
        ipc_degradation_front_message: str = "{:<26} {:<5}".format(
            "IPC DEGRADATION      (%): ",
            str(
                round(
                    level_execution.front_end_percentage_ipc_degradation(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        )
        ipc_degradation_back_message: str = "{:<26} {:<5}".format(
            "IPC DEGRADATION      (%): ",
            str(
                round(
                    level_execution.back_end_percentage_ipc_degradation(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        ...
        messages: list[list[str]] = [
            ["", "", "", ""],
            [
                stalls_front_message,
                stalls_back_message,
                ipc_degradation_divergence_message,
                ipc_retire_message,
            ],
            [ipc_degradation_front_message, ipc_degradation_back_message, " ", " "],
        ]
        titles: list[str] = [
            level_execution.front_end().name(),
            level_execution.back_end().name(),
            level_execution.divergence().name(),
            level_execution.retire().name(),
        ]
        MessageFormat().print_four_msg_box(
            messages, titles, 1, self.output_file(), False
        )

    def __show_level_two_results(self, level_execution: LevelTwo):
        """Show results of level two."""

        stalls_front_decode_on_total_message: str = "{:<20} {:<6}".format(
            "STALLS, on the total (%): ",
            str(
                round(
                    level_execution.front_decode_stall(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        )
        stalls_front_fetch_on_total_message: str = "{:<20} {:<6}".format(
            "STALLS, on the total (%): ",
            str(
                round(
                    level_execution.front_fetch_stall(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        )
        stalls_back_core_bound_on_total_message: str = "{:<20} {:<6}".format(
            "STALLS, on the total (%): ",
            str(
                round(
                    level_execution.back_core_bound_stall(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        )
        stalls_back_memory_bound_on_total_message: str = "{:<20} {:<6}".format(
            "STALLS, on the total (%): ",
            str(
                round(
                    level_execution.back_memory_bound_stall(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        )
        stalls_front_decode_on_front_message: str = "{:<22} {:<6}".format(
            "STALLS, on FrontEnd  (%): ",
            str(
                round(
                    level_execution.front_decode_stall_on_front(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        )
        stalls_front_fetch_on_front_message: str = "{:<20} {:<6}".format(
            "STALLS, on FrontEnd  (%): ",
            str(
                round(
                    level_execution.front_fetch_stall_on_front(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        )
        stalls_back_core_bound_on_back_message: str = "{:<20} {:<6}".format(
            "STALLS, on BackEnd   (%): ",
            str(
                round(
                    level_execution.back_core_bound_stall_on_back(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        )
        stalls_back_memory_bound_on_back_message: str = "{:<20} {:<6}".format(
            "STALLS, on BackEnd   (%): ",
            str(
                round(
                    level_execution.back_memory_bound_stall_on_back(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        )
        ipc_degradation_front_decode_message: str = "{:<26} {:<5}".format(
            "IPC DEGRADATION      (%): ",
            str(
                round(
                    level_execution.front_decode_percentage_ipc_degradation(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        )
        ipc_degradation_front_fetch_message: str = "{:<26} {:<5}".format(
            "IPC DEGRADATION      (%): ",
            str(
                round(
                    level_execution.front_fetch_percentage_ipc_degradation(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        )
        ipc_degradation_back_core_bound_message: str = "{:<26} {:<5}".format(
            "IPC DEGRADATION      (%): ",
            str(
                round(
                    level_execution.back_core_bound_percentage_ipc_degradation(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        )
        ipc_degradation_back_memory_bound_message: str = "{:<26} {:<5}".format(
            "IPC DEGRADATION      (%): ",
            str(
                round(
                    level_execution.back_memory_bound_percentage_ipc_degradation(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        ...
        messages: list[list[str]] = [
            ["", "", "", ""],
            [
                stalls_front_decode_on_total_message,
                stalls_front_fetch_on_total_message,
                stalls_back_core_bound_on_total_message,
                stalls_back_memory_bound_on_total_message,
            ],
            [
                stalls_front_decode_on_front_message,
                stalls_front_fetch_on_front_message,
                stalls_back_core_bound_on_back_message,
                stalls_back_memory_bound_on_back_message,
            ],
            ["", "", "", ""],
            [
                ipc_degradation_front_decode_message,
                ipc_degradation_front_fetch_message,
                ipc_degradation_back_core_bound_message,
                ipc_degradation_back_memory_bound_message,
            ],
        ]
        titles: list[str] = [
            level_execution.front_decode().name(),
            level_execution.front_fetch().name(),
            level_execution.back_core_bound().name(),
            level_execution.back_memory_bound().name(),
        ]
        ...
        ipc_degradation_branch_divergence_message: str = "{:<20} {:<6}".format(
            "IPC DEGRADATION (%): ",
            str(
                round(
                    level_execution.branch_divergence_percentage_ipc_degradation(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        )
        ipc_degradation_replay_divergence_message: str = "{:<20} {:<6}".format(
            "IPC DEGRADATION (%): ",
            str(
                round(
                    level_execution.replay_divergence_percentage_ipc_degradation(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%",
        ...
        titles = [
            level_execution.divergence_branch().name(),
            level_execution.divergence_replay().name(),
        ]
        messages = [
            [
                ipc_degradation_branch_divergence_message,
                ipc_degradation_replay_divergence_message,
            ]
        ]
    ...
    def __show_level_three_results(self, level_execution: LevelThree):
        ...
        stalls_memory_constant_memory_bound_on_total_message: str = (
            "STALLS, on the total             (%): "
            + str(
                round(
                    level_execution.memory_constant_memory_bound_stall(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%"
        )
        stalls_memory_constant_memory_bound_on_memory_bound_message: str = (
            "STALLS, on "
            + level_execution.back_memory_bound().name()
            + " (%): "
            + str(
                round(
                    level_execution.memory_constant_memory_bound_stall_on_memory_bound(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%"
        )
        stalls_memory_constant_memory_bound_on_back_message: str = (
            "STALLS, on "
            + level_execution.back_end().name()
            + "              (%): "
            + str(
                round(
                    level_execution.memory_constant_memory_bound_stall_on_back(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%"
        )
        ipc_degradation_memory_constant_memory_bound_message: str = (
            "IPC DEGRADATION                  (%): "
            + str(
                round(
                    level_execution.memory_constant_memory_bound_percentage_ipc_degradation(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + "%"
        ...
        if type(level_execution) is LevelThreeNsight:
            stalls_memory_mio_throttle_on_total_message: str = (
                "STALLS, on the total             (%): "
                + str(
                    round(
                        level_execution.memory_mio_throttle_stall(),
                        TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                    )
                )
                + "%"
            )
            stalls_memory_l1_bound_on_total_message: str = (
                "STALLS, on the total             (%): "
                + str(
                    round(
                        level_execution.memory_l1_bound_stall(),
                        TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                    )
                )
                + "%"
            )
            stalls_memory_mio_throttle_on_memory_bound_message: str = (
                "STALLS, on "
                + level_execution.back_memory_bound().name()
                + " (%): "
                + str(
                    round(
                        level_execution.memory_mio_throttle_stall_on_memory_bound(),
                        TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                    )
                )
                + "%"
            )
            stalls_memory_l1_bound_on_memory_bound_message: str = (
                "STALLS, on "
                + level_execution.back_memory_bound().name()
                + " (%): "
                + str(
                    round(
                        level_execution.memory_l1_bound_stall_on_memory_bound(),
                        TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                    )
                )
                + "%"
            )
            stalls_memory_mio_throttle_on_back_message: str = (
                "STALLS, on "
                + level_execution.back_end().name()
                + "              (%): "
                + str(
                    round(
                        level_execution.memory_mio_throttle_stall_on_back(),
                        TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                    )
                )
                + "%"
            )
            stalls_memory_l1_bound_on_back_message: str = (
                "STALLS, on "
                + level_execution.back_end().name()
                + "              (%): "
                + str(
                    round(
                        level_execution.memory_l1_bound_stall_on_back(),
                        TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                    )
                )
                + "%"
            )
            ipc_degradation_memory_mio_throttle_message: str = (
                "IPC DEGRADATION                  (%): "
                + str(
                    round(
                        level_execution.memory_mio_throttle_percentage_ipc_degradation(),
                        TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                    )
                )
                + "%"
            )
            ipc_degradation_memory_l1_bound_message: str = (
                "IPC DEGRADATION                  (%): "
                + str(
                    round(
                        level_execution.memory_l1_bound_percentage_ipc_degradation(),
                        TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                    )
                )
                + "%"
            ...
            titles: list[str] = [
                level_execution.memory_constant_memory_bound().name(),
                level_execution.memory_mio_throttle().name(),
                level_execution.memory_l1_bound().name(),
            ]
            messages: list[list[str]] = [
                [
                    stalls_memory_constant_memory_bound_on_total_message,
                    stalls_memory_mio_throttle_on_total_message,
                    stalls_memory_l1_bound_on_total_message,
                ],
                [
                    stalls_memory_constant_memory_bound_on_memory_bound_message,
                    stalls_memory_mio_throttle_on_memory_bound_message,
                    stalls_memory_l1_bound_on_memory_bound_message,
                ],
                [
                    stalls_memory_constant_memory_bound_on_back_message,
                    stalls_memory_mio_throttle_on_back_message,
                    stalls_memory_l1_bound_on_back_message,
                ],
                ["", "", "", ""],
                [
                    ipc_degradation_memory_constant_memory_bound_message,
                    ipc_degradation_memory_mio_throttle_message,
                    ipc_degradation_memory_l1_bound_message,
                ],
            ]
            MessageFormat().print_three_msg_box(
                messages, titles, 1, self.output_file(), False
            )
        else:
            messages: str = (
    ...
    def __show_results(self, level_execution):
        ...
        printer.print_max_line_length_message(
            ...
            max_length=TopDownParameters.C_NUM_MAX_CHARACTERS_PER_LINE,
            output_file=self.output_file(),
            delete_content_file=delete_content,
        )
        print()
        message = (
            "IPC OBTAINED: "
            + str(
                round(
                    level_execution.retire_ipc(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
            )
            + " | MAXIMUM POSSIBLE IPC: "
            + str(level_execution.get_device_max_ipc())
        )
        printer.print_desplazed_msg_box(
            msg=message,
            indent=1,
            title="",
            output_file=self.output_file(),
        ...
        if self.show_desc():
            ...
            ipc_obtained = (
                round(
                    level_execution.get_device_max_ipc() / level_execution.ipc(),
                    TopDownParameters.C_MAX_NUM_RESULTS_DECIMALS,
                )
                if level_execution.ipc()
                else 0
            )
            message = (
                "\n    As you can see, the IPC obtanied it is "
                + str(ipc_obtained * 100)
        ...
        printer.print_desplazed_underlined_str(
            message=message, output_file=self.output_file(), delete_content_file=False
        )
        print()
        if (
            type(level_execution) is LevelTwoNsight
            or type(level_execution) is LevelTwoNvprof
            or type(level_execution) is LevelThreeNvprof
            or type(level_execution) is LevelThreeNsight
        ):
            message = "\nLEVEL ONE RESULTS"
            printer.print_underlined_str(
                message=message,
                output_file=self.output_file(),
                delete_content_file=False,
            )
            ...
            if self.show_desc():
                message = (
                    ...
                    + level_execution.front_end().name()
                    + ": "
                    + level_execution.front_end().description()
                    + "\n\n"
                )
                printer.print_max_line_length_message(
                    message=message,
                ...
                message = (
                    ...
                    + level_execution.back_end().name()
                    + ": "
                    + level_execution.back_end().description()
                    + "\n\n"
                )
                printer.print_max_line_length_message(
                    message=message,
                ...
                message = (
                    ...
                    + level_execution.divergence().name()
                    + ": "
                    + level_execution.divergence().description()
                    + "\n\n"
                )
                printer.print_max_line_length_message(
                    message=message,
                ...
                message = (
                    ...
                    + level_execution.retire().name()
                    + ": "
                    + level_execution.retire().description()
                    + "\n\n"
                )
                printer.print_max_line_length_message(
                    message=message,
            ...
            if self.show_desc():
                message = (
                    ...
                    + level_execution.front_fetch().name()
                    + ": "
                    + level_execution.front_fetch().description()
                    + "\n\n"
                )
                printer.print_max_line_length_message(
                    message=message,
                ...
                message = (
                    ...
                    + level_execution.front_decode().name()
                    + ": "
                    + level_execution.front_decode().description()
                    + "\n\n"
                )
                printer.print_max_line_length_message(
                    message=message,
                ...
                message = (
                    ...
                    + level_execution.back_core_bound().name()
                    + ": "
                    + level_execution.back_core_bound().description()
                    + "\n\n"
                )
                printer.print_max_line_length_message(
                    message=message,
                ...
                message = (
                    ...
                    + level_execution.back_memory_bound().name()
                    + ": "
                    + level_execution.back_memory_bound().description()
                    + "\n\n"
                )
                printer.print_max_line_length_message(
                    message=message,
                ...
                message = (
                    ...
                    + level_execution.divergence_branch().name()
                    + ": "
                    + level_execution.divergence_branch().description()
                    + "\n\n"
                )
                printer.print_max_line_length_message(
                    message=message,
                ...
                message = (
                    ...
                    + level_execution.divergence_replay().name()
                    + ": "
                    + level_execution.divergence_replay().description()
                    + "\n\n"
                )
                printer.print_max_line_length_message(
                    message=message,
            ...
            if (
                ...
                if self.show_desc():
                    message = (
                        ...
                        + level_execution.memory_constant_memory_bound().name()
                        + ": "
                        + level_execution.memory_constant_memory_bound().description()
                        + "\n\n"
                    )
                    printer.print_max_line_length_message(
                        message=message,
                    ...
                    if type(level_execution) is LevelThreeNsight:
                        message = (
                            ...
                            + level_execution.memory_l1_bound().name()
                            + ": "
                            + level_execution.memory_l1_bound().description()
                            + "\n\n"
                        )
                        printer.print_max_line_length_message(
                            message=message,
                        ...
                        message = (
                            ...
                            + level_execution.memory_mio_throttle().name()
                            + ": "
                            + level_execution.memory_mio_throttle().description()
                            + "\n\n"
                        )
                        printer.print_max_line_length_message(
                            message=message,
    ...
    def __is_nvprof_mode(self) -> bool:
        ...
        shell: Shell = Shell()
        compute_capability: str = shell.launch_command_show_all(
            "nvcc $DIR_UNTIL_TOPDOWN/TopDownNvidia/src/measure_parts/compute_capability.cu --run",
            None,
        )
        shell.launch_command(
            "rm -f $DIR_UNTIL_TOPDOWN/TopDownNvidia/src/measure_parts/a.out", None
        )  # delete file
        ...
        if (
            compute_capability_float
            > TopDownParameters.C_COMPUTE_CAPABILITY_NVPROF_MAX_VALUE
        ):
            return False
        return True
    ...
    def launch(self):
        ...
        if (
            program is not None
            and len(program) > 3
            and program[len(program) - 3] == "."
            and program[len(program) - 2] == "p"
        ...
        if self.__is_nvprof_mode():
            ...
            if self.level() == 1:
                front_end = FrontEndNvprof(
                    FrontEndParameters.C_FRONT_END_NAME,
                    FrontEndParameters.C_FRONT_END_DESCRIPTION,
                    FrontEndParameters.C_FRONT_END_NVPROF_L1_METRICS,
                    FrontEndParameters.C_FRONT_END_NVPROF_L1_EVENTS,
                )
                back_end = BackEndNvprof(
                    BackEndParameters.C_BACK_END_NAME,
                    BackEndParameters.C_BACK_END_DESCRIPTION,
                    BackEndParameters.C_BACK_END_NVPROF_L1_METRICS,
                    BackEndParameters.C_BACK_END_NVPROF_L1_EVENTS,
                )
                divergence = DivergenceNvprof(
                    DivergenceParameters.C_DIVERGENCE_NAME,
                    DivergenceParameters.C_DIVERGENCE_DESCRIPTION,
                    DivergenceParameters.C_DIVERGENCE_NVPROF_L1_METRICS,
                    DivergenceParameters.C_DIVERGENCE_NVPROF_L1_EVENTS,
                )
                retire = RetireNvprof(
                    RetireParameters.C_RETIRE_NAME,
                    RetireParameters.C_RETIRE_DESCRIPTION,
                    RetireParameters.C_RETIRE_NVPROF_L1_METRICS,
                    RetireParameters.C_RETIRE_NVPROF_L1_EVENTS,
                )
                extra_measure = ExtraMeasureNvprof(
                    ExtraMeasureParameters.C_EXTRA_MEASURE_NAME,
                    ExtraMeasureParameters.C_EXTRA_MEASURE_DESCRIPTION,
                    ExtraMeasureParameters.C_EXTRA_MEASURE_NVPROF_L1_METRICS,
                    ExtraMeasureParameters.C_EXTRA_MEASURE_NVPROF_L1_EVENTS,
                )
                level: LevelOneNvprof = LevelOneNvprof(
                    program,
                    self.input_file(),
                    self.output_file(),
                    self.output_scan_file(),
                    show_metrics,
                    show_events,
                    front_end,
                    back_end,
                ...
                front_end = FrontEndNvprof(
                    FrontEndParameters.C_FRONT_END_NAME,
                    FrontEndParameters.C_FRONT_END_DESCRIPTION,
                    FrontEndParameters.C_FRONT_END_NVPROF_L2_METRICS,
                    FrontEndParameters.C_FRONT_END_NVPROF_L2_EVENTS,
                )
                back_end = BackEndNvprof(
                    BackEndParameters.C_BACK_END_NAME,
                    BackEndParameters.C_BACK_END_DESCRIPTION,
                    BackEndParameters.C_BACK_END_NVPROF_L2_METRICS,
                    BackEndParameters.C_BACK_END_NVPROF_L2_EVENTS,
                )
                divergence = DivergenceNvprof(
                    DivergenceParameters.C_DIVERGENCE_NAME,
                    DivergenceParameters.C_DIVERGENCE_DESCRIPTION,
                    DivergenceParameters.C_DIVERGENCE_NVPROF_L2_METRICS,
                    DivergenceParameters.C_DIVERGENCE_NVPROF_L2_EVENTS,
                )
                retire = RetireNvprof(
                    RetireParameters.C_RETIRE_NAME,
                    RetireParameters.C_RETIRE_DESCRIPTION,
                    RetireParameters.C_RETIRE_NVPROF_L2_METRICS,
                    RetireParameters.C_RETIRE_NVPROF_L2_EVENTS,
                )
                extra_measure = ExtraMeasureNvprof(
                    ExtraMeasureParameters.C_EXTRA_MEASURE_NAME,
                    ExtraMeasureParameters.C_EXTRA_MEASURE_DESCRIPTION,
                    ExtraMeasureParameters.C_EXTRA_MEASURE_NVPROF_L2_METRICS,
                    ExtraMeasureParameters.C_EXTRA_MEASURE_NVPROF_L2_EVENTS,
                )
                front_decode: FrontDecodeNvprof = FrontDecodeNvprof(
                    FrontDecodeParameters.C_FRONT_DECODE_NAME,
                    FrontDecodeParameters.C_FRONT_DECODE_DESCRIPTION,
                    FrontDecodeParameters.C_FRONT_DECODE_NVPROF_L2_METRICS,
                    FrontDecodeParameters.C_FRONT_DECODE_NVPROF_L2_EVENTS,
                )
                front_fetch: FrontFetchNvprof = FrontFetchNvprof(
                    FrontFetchParameters.C_FRONT_FETCH_NAME,
                    FrontFetchParameters.C_FRONT_FETCH_DESCRIPTION,
                    FrontFetchParameters.C_FRONT_FETCH_NVPROF_L2_METRICS,
                    FrontFetchParameters.C_FRONT_FETCH_NVPROF_L2_EVENTS,
                )
                back_memory_bound: BackMemoryBoundNvprof = BackMemoryBoundNvprof(
                    BackMemoryBoundParameters.C_BACK_MEMORY_BOUND_NAME,
                    BackMemoryBoundParameters.C_BACK_MEMORY_BOUND_DESCRIPTION,
                    BackMemoryBoundParameters.C_BACK_MEMORY_BOUND_NVPROF_L2_METRICS,
                    BackMemoryBoundParameters.C_BACK_MEMORY_BOUND_NVPROF_L2_EVENTS,
                )
                back_core_bound: BackCoreBoundNvprof = BackCoreBoundNvprof(
                    BackCoreBoundParameters.C_BACK_CORE_BOUND_NAME,
                    BackCoreBoundParameters.C_BACK_CORE_BOUND_DESCRIPTION,
                    BackCoreBoundParameters.C_BACK_CORE_BOUND_NVPROF_L2_METRICS,
                    BackCoreBoundParameters.C_BACK_CORE_BOUND_NVPROF_L2_EVENTS,
                )
                level: LevelTwoNvprof = LevelTwoNvprof(
                    program,
                    self.input_file(),
                    self.output_file(),
                    self.output_scan_file(),
                ...
                front_end = FrontEndNvprof(
                    FrontEndParameters.C_FRONT_END_NAME,
                    FrontEndParameters.C_FRONT_END_DESCRIPTION,
                    FrontEndParameters.C_FRONT_END_NVPROF_L3_METRICS,
                    FrontEndParameters.C_FRONT_END_NVPROF_L3_EVENTS,
                )
                back_end = BackEndNvprof(
                    BackEndParameters.C_BACK_END_NAME,
                    BackEndParameters.C_BACK_END_DESCRIPTION,
                    BackEndParameters.C_BACK_END_NVPROF_L3_METRICS,
                    BackEndParameters.C_BACK_END_NVPROF_L3_EVENTS,
                )
                divergence = DivergenceNvprof(
                    DivergenceParameters.C_DIVERGENCE_NAME,
                    DivergenceParameters.C_DIVERGENCE_DESCRIPTION,
                    DivergenceParameters.C_DIVERGENCE_NVPROF_L3_METRICS,
                    DivergenceParameters.C_DIVERGENCE_NVPROF_L3_EVENTS,
                )
                retire = RetireNvprof(
                    RetireParameters.C_RETIRE_NAME,
                    RetireParameters.C_RETIRE_DESCRIPTION,
                    RetireParameters.C_RETIRE_NVPROF_L3_METRICS,
                    RetireParameters.C_RETIRE_NVPROF_L3_EVENTS,
                )
                extra_measure = ExtraMeasureNvprof(
                    ExtraMeasureParameters.C_EXTRA_MEASURE_NAME,
                    ExtraMeasureParameters.C_EXTRA_MEASURE_DESCRIPTION,
                    ExtraMeasureParameters.C_EXTRA_MEASURE_NVPROF_L3_METRICS,
                    ExtraMeasureParameters.C_EXTRA_MEASURE_NVPROF_L3_EVENTS,
                )
                front_decode: FrontDecodeNvprof = FrontDecodeNvprof(
                    FrontDecodeParameters.C_FRONT_DECODE_NAME,
                    FrontDecodeParameters.C_FRONT_DECODE_DESCRIPTION,
                    FrontDecodeParameters.C_FRONT_DECODE_NVPROF_L3_METRICS,
                    FrontDecodeParameters.C_FRONT_DECODE_NVPROF_L3_EVENTS,
                )
                front_fetch: FrontFetchNvprof = FrontFetchNvprof(
                    FrontFetchParameters.C_FRONT_FETCH_NAME,
                    FrontFetchParameters.C_FRONT_FETCH_DESCRIPTION,
                    FrontFetchParameters.C_FRONT_FETCH_NVPROF_L3_METRICS,
                    FrontFetchParameters.C_FRONT_FETCH_NVPROF_L3_EVENTS,
                )
                back_memory_bound: BackMemoryBoundNvprof = BackMemoryBoundNvprof(
                    BackMemoryBoundParameters.C_BACK_MEMORY_BOUND_NAME,
                    BackMemoryBoundParameters.C_BACK_MEMORY_BOUND_DESCRIPTION,
                    BackMemoryBoundParameters.C_BACK_MEMORY_BOUND_NVPROF_L3_METRICS,
                    BackMemoryBoundParameters.C_BACK_MEMORY_BOUND_NVPROF_L3_EVENTS,
                )
                back_core_bound: BackCoreBoundNvprof = BackCoreBoundNvprof(
                    BackCoreBoundParameters.C_BACK_CORE_BOUND_NAME,
                    BackCoreBoundParameters.C_BACK_CORE_BOUND_DESCRIPTION,
                    BackCoreBoundParameters.C_BACK_CORE_BOUND_NVPROF_L3_METRICS,
                    BackCoreBoundParameters.C_BACK_CORE_BOUND_NVPROF_L3_EVENTS,
                )
                level: LevelThreeNvprof = LevelThreeNvprof(
                    program,
                    self.input_file(),
                    self.output_file(),
                    self.output_scan_file(),
            ...
            if self.level() == 1:
                ...
                level: LevelOneNsight = LevelOneNsight(
                    program,
                    self.input_file(),
                    self.output_file(),
                    self.output_scan_file(),
                ...
                front_decode: FrontDecodeNsight = FrontDecodeNsight(
                    FrontDecodeParameters.C_FRONT_DECODE_NAME,
                    FrontDecodeParameters.C_FRONT_DECODE_DESCRIPTION,
                    FrontDecodeParameters.C_FRONT_DECODE_NSIGHT_L2_METRICS,
                )
                front_fetch: FrontFetchNsight = FrontFetchNsight(
                    FrontFetchParameters.C_FRONT_FETCH_NAME,
                    FrontFetchParameters.C_FRONT_FETCH_DESCRIPTION,
                    FrontFetchParameters.C_FRONT_FETCH_NSIGHT_L2_METRICS,
                )
                back_memory_bound: BackMemoryBoundNsight = BackMemoryBoundNsight(
                    BackMemoryBoundParameters.C_BACK_MEMORY_BOUND_NAME,
                    BackMemoryBoundParameters.C_BACK_MEMORY_BOUND_DESCRIPTION,
                    BackMemoryBoundParameters.C_BACK_MEMORY_BOUND_NSIGHT_L2_METRICS,
                )
                back_core_bound: BackCoreBoundNsight = BackCoreBoundNsight(
                    BackCoreBoundParameters.C_BACK_CORE_BOUND_NAME,
                    BackCoreBoundParameters.C_BACK_CORE_BOUND_DESCRIPTION,
                    BackCoreBoundParameters.C_BACK_CORE_BOUND_NSIGHT_L2_METRICS,
                )
                level: LevelTwoNsight = LevelTwoNsight(
                    program,
                    self.input_file(),
                    self.output_file(),
                    self.output_scan_file(),
                ...
                front_decode: FrontDecodeNsight = FrontDecodeNsight(
                    FrontDecodeParameters.C_FRONT_DECODE_NAME,
                    FrontDecodeParameters.C_FRONT_DECODE_DESCRIPTION,
                    FrontDecodeParameters.C_FRONT_DECODE_NSIGHT_L3_METRICS,
                )
                front_fetch: FrontFetchNsight = FrontFetchNsight(
                    FrontFetchParameters.C_FRONT_FETCH_NAME,
                    FrontFetchParameters.C_FRONT_FETCH_DESCRIPTION,
                    FrontFetchParameters.C_FRONT_FETCH_NSIGHT_L3_METRICS,
                )
                back_memory_bound: BackMemoryBoundNsight = BackMemoryBoundNsight(
                    BackMemoryBoundParameters.C_BACK_MEMORY_BOUND_NAME,
                    BackMemoryBoundParameters.C_BACK_MEMORY_BOUND_DESCRIPTION,
                ...
                level: LevelThreeNsight = LevelThreeNsight(
                    program,
                    self.input_file(),
                    self.output_file(),
                    self.output_scan_file(),
...
if __name__ == "__main__":
    td = TopDown()
    td.launch()
    MessageFormat().print_max_line_length_message(
