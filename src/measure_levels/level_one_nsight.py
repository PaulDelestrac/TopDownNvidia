...
class LevelOneNsight(LevelOne, LevelExecutionNsight):
    ...
    def _divergence_ipc_degradation(self) -> float:
        ...
        """
        return super()._diver_ipc_degradation(
    ...
    def _set_front_back_divergence_retire_results(self, results_launch: str):
        ...
        for line in str(results_launch).splitlines():
            ...
            if (len(list_words) == 4 or len(list_words) == 3) and list_words[1][
                ...
                front_end_value_has_found = self._front_end.set_metric_value(
                    metric_name, metric_value
                )
                frond_end_unit_has_found = self._front_end.set_metric_unit(
                    metric_name, metric_unit
                )
                back_end_value_has_found = self._back_end.set_metric_value(
                    metric_name, metric_value
                )
                back_end_unit_has_found = self._back_end.set_metric_unit(
                    metric_name, metric_unit
                )
                divergence_value_has_found = self._divergence.set_metric_value(
                    metric_name, metric_value
                )
                divergence_unit_has_found = self._divergence.set_metric_unit(
                    metric_name, metric_unit
                )
                extra_measure_value_has_found = self._extra_measure.set_metric_value(
                    metric_name, metric_value
                )
                extra_measure_unit_has_found = self._extra_measure.set_metric_unit(
                    metric_name, metric_unit
                )
                retire_value_has_found = self._retire.set_metric_value(
                    metric_name, metric_value
                )
                retire_unit_has_found = self._retire.set_metric_unit(
                    metric_name, metric_unit
                )
                if not (
                    front_end_value_has_found
                    or back_end_value_has_found
                    or divergence_value_has_found
                    or extra_measure_value_has_found
                    or retire_value_has_found
                ) or not (
                    frond_end_unit_has_found
                    or back_end_unit_has_found
                    or divergence_unit_has_found
                    or extra_measure_unit_has_found
                    or retire_unit_has_found
                ):
