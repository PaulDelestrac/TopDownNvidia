...
class LevelTwo(LevelOne, ABC):
    ...
    def back_core_bound_percentage_ipc_degradation(self) -> float:
        ...
        return (
            (
                (self._stall_ipc() * (self.back_core_bound_stall() / 100.0))
    ...
    def back_memory_bound_percentage_ipc_degradation(self) -> float:
        ...
        return (
            (
                (self._stall_ipc() * (self.back_memory_bound_stall() / 100.0))
                / super().get_device_max_ipc()
            )
            * 100.0
            if super().get_device_max_ipc()
    ...
    def front_decode_percentage_ipc_degradation(self) -> float:
        ...
        return (
            (
                (self._stall_ipc() * (self.front_decode_stall() / 100.0))
                / super().get_device_max_ipc()
            )
            * 100.0
            if super().get_device_max_ipc()
    ...
    def front_fetch_percentage_ipc_degradation(self) -> float:
        ...
        return (
            (
                (self._stall_ipc() * (self.front_fetch_stall() / 100.0))
                / super().get_device_max_ipc()
            )
            * 100.0
            if super().get_device_max_ipc()
    ...
    def total_core_memory_stall(self) -> float:
        ...
        back_memory: float = super()._get_stalls_of_part(
            self.back_memory_bound().metrics()
        )
    ...
    def back_memory_bound_stall(self) -> float:
        ...
        return (
            (
                super()._get_stalls_of_part(self._back_memory_bound.metrics())
                / super().total_front_back_stall()
            )
            * 100.0
            if super().total_front_back_stall()
            else 0
        )
        pass
    ...
    def back_core_bound_stall(self) -> float:
        ...
        return (
            (
                super()._get_stalls_of_part(self._back_core_bound.metrics())
                / super().total_front_back_stall()
            )
            * 100.0
            if super().total_front_back_stall()
    ...
    def front_decode_stall(self) -> float:
        ...
        return (
            (
                super()._get_stalls_of_part(self._front_decode.metrics())
                / super().total_front_back_stall()
            )
            * 100.0
            if super().total_front_back_stall()
    ...
    def front_fetch_stall(self) -> float:
        ...
        return (
            (
                super()._get_stalls_of_part(self._front_fetch.metrics())
                / super().total_front_back_stall()
            )
            * 100.0
            if super().total_front_back_stall()
    ...
    def back_memory_bound_stall_on_back(self) -> float:
        ...
        return (
            (
                super()._get_stalls_of_part(self._back_memory_bound.metrics())
                / self.total_core_memory_stall()
            )
            * 100.0
            if self.total_core_memory_stall()
            else 0
        )
    ...
    def front_decode_stall_on_front(self) -> float:
        ...
        return (
            (
                super()._get_stalls_of_part(self._front_decode.metrics())
                / self.total_fetch_decode_stall()
            )
            * 100.0
            if self.total_fetch_decode_stall()
            else 0
        )
        pass
    ...
    def branch_divergence_percentage_ipc_degradation(self) -> float:
        ...
        return (
            (self._branch_divergence_ipc_degradation() / super().get_device_max_ipc())
            * 100.0
            if super().get_device_max_ipc()
            else 0
        )
    ...
    def replay_divergence_percentage_ipc_degradation(self) -> float:
        ...
        return (
            (self._replay_divergence_ipc_degradation() / super().get_device_max_ipc())
            * 100.0
            if super().get_device_max_ipc()
            else 0
        )
    ...
    def _create_graph(self) -> PieChart:
        ...
        titles_graphs: list = LevelExecutionParameters.level_two_graphs_titles(
            self._front_end.name(), self._back_end.name()
        )
        if len(titles_graphs) < 6:
            raise TitleSizeError
        return PieChart(
            3, 2, "Description of Results", titles_graphs
        )  # pie chart graph
        pass
    ...
    def _add_graph_data(self, graph: PieChart):
        ...
        labels: list = [
            ...
            self._front_decode.name(),
            self._front_fetch.name(),
            self._back_end.name(),
            self._back_core_bound.name(),
            self._back_memory_bound.name(),
        ...
        # Level One
        values: list = [
            super().front_end_percentage_ipc_degradation(),
            None,
            None,
            super().back_end_percentage_ipc_degradation(),
            None,
            super().divergence_percentage_ipc_degradation(),
            super().retire_ipc(),
        ]
        ...
        values = [
            None,
            self.front_fetch_percentage_ipc_degradation(),
            self.front_decode_percentage_ipc_degradation(),
            None,
            self.back_core_bound_percentage_ipc_degradation(),
            self.back_memory_bound_percentage_ipc_degradation(),
            super().divergence_percentage_ipc_degradation(),
            super().retire_ipc(),
        ]  # IPC Degradation
        graph.add_graph(labels, values, "1")
        ...
        values = [
            None,
            self.front_decode_stall(),
            self.front_fetch_stall(),
            None,
            self.back_core_bound_stall(),
            self.back_memory_bound_stall(),
            None,
            None,
        ]  # Stalls total
        graph.add_graph(labels, values, "1")
        ...
        values = [
            None,
            self.front_decode_stall_on_front(),
            self.front_fetch_stall_on_front(),
            None,
            None,
            None,
            None,
        ...
        values = [
            ...
            None,
            None,
            self.back_core_bound_stall_on_back(),
            self.back_memory_bound_stall_on_back(),
            None,
            None,
        ]  # Stalls on BackEnd
        graph.add_graph(labels, values, "1")
