...
class LevelTwoNvprof(LevelTwo, LevelOneNvprof):
    ...
    def __init__(
        ...
        front_decode: FrontDecodeNvprof,
        front_fetch: FrontFetchNvprof,
        back_core_bound: BackCoreBoundNvprof,
        back_memory_bound: BackMemoryBoundNvprof,
    ):
        ...
        self._branch_divergence: DivergenceBranchNvprof = DivergenceBranchNvprof(
            DivergenceBranchParameters.C_DIVERGENCE_BRANCH_NAME,
            DivergenceBranchParameters.C_DIVERGENCE_BRANCH_DESCRIPTION,
            "",
            "",
        )
        self._replay_divergence: DivergenceReplayNvprof = DivergenceReplayNvprof(
            DivergenceReplayParameters.C_DIVERGENCE_REPLAY_NAME,
    ...
    def _generate_command(self) -> str:
        ...
        command: str = (
            "nvprof --metrics "
            + self._front_end.metrics_str()
            + ","
            + self._back_end.metrics_str()
            + ","
            + self._divergence.metrics_str()
            + ","
            + self._extra_measure.metrics_str()
            + ","
            + self._retire.metrics_str()
            + ","
            + self._front_decode.metrics_str()
            + ","
            + self._front_fetch.metrics_str()
            + ","
            + self._back_core_bound.metrics_str()
            + ","
            + self._back_memory_bound.metrics_str()
            + "  --events "
            + self._front_end.events_str()
            + ","
            + self._back_end.events_str()
            + ","
            + self._divergence.events_str()
            + ","
            + self._extra_measure.events_str()
            + ","
            + self._retire.events_str()
            + ","
            + self._front_decode.events_str()
            + ","
            + self._front_fetch.events_str()
            + ","
            + self._back_core_bound.events_str()
            + ","
            + self._back_memory_bound.events_str()
    ...
    def _get_results(self, lst_output: list):
        ...
        if (
            ...
            and self._front_end.metrics_str() != ""
            or self._collect_events
            and self._front_end.events_str() != ""
        ):
            lst_output.append(converter.underlined_str(self._front_end.name()))
        if self._collect_metrics and self._front_end.metrics_str() != "":
            super()._add_result_part_to_lst(
                self._front_end.metrics(),
        ...
        if self._collect_events and self._front_end.events_str() != "":
            super()._add_result_part_to_lst(
                self._front_end.events(),
                self._front_end.events_description(),
                "",
                lst_output,
                False,
            )
        if (
            self._collect_metrics
            and self._front_decode.metrics_str() != ""
            or self._collect_events
            and self._front_decode.events_str() != ""
        ):
        ...
        if self._collect_metrics and self._front_decode.metrics_str() != "":
            super()._add_result_part_to_lst(
                self._front_decode.metrics(),
                self._front_decode.metrics_description(),
                lst_output,
                True,
            )
        if self._collect_events and self._front_decode.events_str() != "":
            super()._add_result_part_to_lst(
                self._front_decode.events(),
                self._front_decode.events_description(),
                lst_output,
                False,
            )
        if (
            self._collect_metrics
            and self._front_fetch.metrics_str() != ""
            or self._collect_events
            and self._front_fetch.events_str() != ""
        ):
        ...
        if self._collect_metrics and self._front_fetch.metrics_str() != "":
            super()._add_result_part_to_lst(
                self._front_fetch.metrics(),
                self._front_fetch.metrics_description(),
                lst_output,
                True,
            )
        if self._collect_events and self._front_fetch.events_str() != "":
            super()._add_result_part_to_lst(
                self._front_fetch.events(),
                self._front_fetch.events_description(),
                lst_output,
                False,
            )
        if (
            self._collect_metrics
            and self._back_end.metrics_str() != ""
            or self._collect_events
            and self._back_end.events_str() != ""
        ):
            lst_output.append(converter.underlined_str(self._back_end.name()))
        if self._collect_metrics and self._back_end.metrics_str() != "":
            super()._add_result_part_to_lst(
                self._back_end.metrics(),
        ...
        if self._collect_events and self._back_end.events_str() != "":
            super()._add_result_part_to_lst(
                self._back_end.events(),
                self._back_end.events_description(),
                lst_output,
                False,
            )
        if (
            self._collect_metrics
            and self._back_core_bound.metrics_str() != ""
            or self._collect_events
            and self._back_core_bound.events_str() != ""
        ):
        ...
        if self._collect_metrics and self._back_core_bound.metrics_str() != "":
            super()._add_result_part_to_lst(
                self._back_core_bound.metrics(),
                self._back_core_bound.metrics_description(),
                lst_output,
                True,
            )
        if self._collect_events and self._back_core_bound.events_str() != "":
            super()._add_result_part_to_lst(
                self._back_core_bound.events(),
                self._back_core_bound.events_description(),
                lst_output,
                False,
            )
        if (
            self._collect_metrics
            and self._back_memory_bound.metrics_str() != ""
            or self._collect_events
            and self._back_memory_bound.events_str() != ""
        ):
        ...
        if self._collect_metrics and self._back_memory_bound.metrics_str() != "":
            super()._add_result_part_to_lst(
                self._back_memory_bound.metrics(),
                self._back_memory_bound.metrics_description(),
                lst_output,
                True,
            )
        if self._collect_events and self._back_memory_bound.events_str() != "":
            super()._add_result_part_to_lst(
                self._back_memory_bound.events(),
                self._back_memory_bound.events_description(),
                lst_output,
                False,
            )
        if (
            self._collect_metrics
            and self._divergence.metrics_str() != ""
            or self._collect_events
            and self._divergence.events_str() != ""
        ):
        ...
        if self._collect_events and self._divergence.events_str() != "":
            super()._add_result_part_to_lst(
                self._divergence.events(),
                self._divergence.events_description(),
                lst_output,
                False,
            )
        if (
            self._collect_metrics
            and self._retire.metrics_str() != ""
            or self._collect_events
            and self._retire.events_str() != ""
        ):
            lst_output.append(converter.underlined_str(self._retire.name()))
        if self._collect_metrics and self._retire.metrics_str() != "":
            super()._add_result_part_to_lst(
                self._retire.metrics(),
        ...
        if self._collect_events and self._retire.events_str() != "":
            super()._add_result_part_to_lst(
                self._retire.events(),
                self._retire.events_description(),
                lst_output,
                False,
            )
        if (
            self._collect_metrics
            and self._extra_measure.metrics_str() != ""
            or self._collect_events
            and self._extra_measure.events_str() != ""
        ):
        ...
        if self._collect_events and self._extra_measure.events_str() != "":
            super()._add_result_part_to_lst(
                self._extra_measure.events(),
                self._extra_measure.events_description(),
                lst_output,
                False,
            )
        lst_output.append("\n")
    ...
    def _set_memory_core_decode_fetch_results(self, results_launch: str):
        ...
        for line in results_launch.splitlines():
            line = re.sub(
                " +", " ", line
            )  # delete more than one spaces and put only one
            ...
            if not has_read_all_events:
                ...
                if (
                    ...
                    back_core_bound_value_has_found = (
                        self._back_core_bound.set_metric_value(
                            metric_name, metric_avg_value
                        )
                    )
                    back_core_bound_description_has_found = (
                        self._back_core_bound.set_metric_description(
                            metric_name, metric_description
                        )
                    )
                    back_memory_bound_value_has_found = (
                        self._back_memory_bound.set_metric_value(
                            metric_name, metric_avg_value
                        )
                    )
                    back_memory_bound_description_has_found = (
                        self._back_memory_bound.set_metric_description(
                            metric_name, metric_description
                        )
                    )
                    front_decode_value_has_found = self._front_decode.set_metric_value(
                        metric_name, metric_avg_value
                    )
                    front_decode_description_has_found = (
                        self._front_decode.set_metric_description(
                            metric_name, metric_description
                        )
                    )
                    front_fetch_value_has_found = self._front_fetch.set_metric_value(
                        metric_name, metric_avg_value
                    )
                    front_fetch_description_has_found = (
                        self._front_fetch.set_metric_description(
                            metric_name, metric_description
                        )
                    )
                    if not (
                        ...
                        back_core_bound_description_has_found
                        or back_memory_bound_description_has_found
                        or front_decode_description_has_found
                        or front_fetch_description_has_found
                    ):
    ...
    def _branch_divergence_ipc_degradation(self) -> float:
        ...
        return super()._branch_diver_ipc_degradation(
            LevelExecutionParameters.C_WARP_EXECUTION_EFFICIENCY_METRIC_NAME_NVPROF
        )
    ...
    def _replay_divergence_ipc_degradation(self) -> float:
        ...
        return super()._replay_diver_ipc_degradation(
    ...
    def _metricExists(self, metric_name: str) -> bool:
        ...
        is_defined_front_end_value: str = (
            super().front_end().get_metric_value(metric_name)
        )
        is_defined_front_end_description: str = (
            super().front_end().get_metric_description(metric_name)
        )

        is_defined_back_end_value: str = (
            super().back_end().get_metric_value(metric_name)
        )
        is_defined_back_end_description: str = (
            super().back_end().get_metric_description(metric_name)
        )

        is_defined_divergence_value: str = (
            super().divergence().get_metric_value(metric_name)
        )
        is_defined_divergence_description: str = (
            super().divergence().get_metric_description(metric_name)
        )

        is_defined_retire_value: str = super().retire().get_metric_value(metric_name)
        is_defined_retire_description: str = (
            super().retire().get_metric_description(metric_name)
        )

        is_defined_extra_measure_value: str = (
            super().extra_measure().get_metric_value(metric_name)
        )
        is_defined_extra_measure_description: str = (
            super().extra_measure().get_metric_description(metric_name)
        )

        if not (
            (
                is_defined_front_end_value is None
                and is_defined_front_end_description is None
            )
            or (
                is_defined_back_end_value is None
                and is_defined_back_end_description is None
            )
            or (
                is_defined_divergence_value is None
                and is_defined_divergence_description is None
            )
            or (
                is_defined_retire_value is None or is_defined_retire_description is None
            )
            or (
                is_defined_extra_measure_value is None
                and is_defined_extra_measure_description is None
            )
        ):
            return False
        return True
    ...
    def __eventExists(self, event_name: str) -> bool:
        ...
        is_defined_front_end_value: str = (
            super().front_end().get_metric_value(event_name)
        )
        is_defined_front_end_description: str = (
            super().front_end().get_metric_description(event_name)
        )

        is_defined_back_end_value: str = super().back_end().get_metric_value(event_name)
        is_defined_back_end_description: str = (
            super().back_end().get_metric_description(event_name)
        )

        is_defined_divergence_value: str = (
            super().divergence().get_metric_value(event_name)
        )
        is_defined_divergence_description: str = (
            super().divergence().get_metric_description(event_name)
        )

        is_defined_retire_value: str = super().retire().get_metric_value(event_name)
        is_defined_retire_description: str = (
            super().retire().get_metric_description(event_name)
        )

        is_defined_extra_measure_value: str = (
            super().extra_measure().get_metric_value(event_name)
        )
        is_defined_extra_measure_description: str = (
            super().extra_measure().get_metric_description(event_name)
        )
