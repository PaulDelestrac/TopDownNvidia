...
from parameters.memory_constant_memory_bound_params import (
    MemoryConstantMemoryBoundParameters,
)
...
class LevelThreeNvprof(LevelThree, LevelTwoNvprof):
    ...
    def _generate_command(self) -> str:
        ...
        command: str = (
            ...
            + ","
            + self.__memory_constant_memory_bound.metrics_str()
            + "  --events "
            ...
            + ","
            + self.__memory_constant_memory_bound.events_str()
    ...
    def _get_results(self, lst_output: list):
        ...
        if (
            self._collect_metrics
            and self.__memory_constant_memory_bound.metrics_str() != ""
            or self._collect_events
            and self.__memory_constant_memory_bound.events_str() != ""
        ):
            lst_output.append(
                converter.underlined_str(self.__memory_constant_memory_bound.name())
            )
        if (
            self._collect_metrics
            and self.__memory_constant_memory_bound.metrics_str() != ""
            ...
            super()._add_result_part_to_lst(
                self.__memory_constant_memory_bound.metrics(),
                self.__memory_constant_memory_bound.metrics_description(),
                lst_output,
                True,
            )
        if (
            ...
            super()._add_result_part_to_lst(
                self.__memory_constant_memory_bound.events(),
                self.__memory_constant_memory_bound.events_description(),
                lst_output,
                False,
            )
        if (
    ...
    def __eventExists(self, event_name: str) -> bool:
        ...
        is_defined_front_fetch_value: str = (
            super().front_fetch().get_event_value(event_name)
        )
        is_defined_front_fetch_description: str = (
            super().front_fetch().get_event_description(event_name)
        )

        is_defined_front_decode_value: str = (
            super().front_decode().get_event_value(event_name)
        )
        is_defined_front_decode_description: str = (
            super().front_decode().get_event_description(event_name)
        )

        is_defined_back_memory_bound_value: str = (
            super().back_memory_bound().get_event_value(event_name)
        )
        is_defined_back_memory_bound_description: str = (
            super().back_memory_bound().get_event_description(event_name)
        )

        is_defined_back_core_bound_value: str = (
            super().back_core_bound().get_event_value(event_name)
        )
        is_defined_back_core_bound_description: str = (
            super().back_core_bound().get_event_description(event_name)
        )

        if not (
            (
                is_defined_front_fetch_value is None
                and is_defined_front_fetch_description is None
            )
            or (
                is_defined_front_decode_value is None
                and is_defined_front_decode_description is None
            )
            or (
                is_defined_back_memory_bound_value is None
                and is_defined_back_memory_bound_description is None
            )
            or (
                is_defined_back_core_bound_value is None
                or is_defined_back_core_bound_description is None
            )
        ):
            return False
        return True
    ...
    def _metricExists(self, metric_name: str) -> bool:
        ...
        is_defined_front_fetch_value: str = (
            super().front_fetch().get_metric_value(metric_name)
        )
        is_defined_front_fetch_description: str = (
            super().front_fetch().get_metric_description(metric_name)
        )

        is_defined_front_decode_value: str = (
            super().front_decode().get_metric_value(metric_name)
        )
        is_defined_front_decode_description: str = (
            super().front_decode().get_metric_description(metric_name)
        )

        is_defined_back_memory_bound_value: str = (
            super().back_memory_bound().get_metric_value(metric_name)
        )
        is_defined_back_memory_bound_description: str = (
            super().back_memory_bound().get_metric_description(metric_name)
        )

        is_defined_back_core_bound_value: str = (
            super().back_core_bound().get_metric_value(metric_name)
        )
        is_defined_back_core_bound_description: str = (
            super().back_core_bound().get_metric_description(metric_name)
        )
            ...
            if not has_read_all_events:
                ...
                if len(list_words) > 1:
                    if list_words[1] == "Metric":  # check end events
                        ...
                        constant_memory_bound_value_has_found = (
                            self.__memory_constant_memory_bound.set_event_value(
                ...
                if (
                    ...
                    constant_memory_bound_value_has_found = (
                        self.__memory_constant_memory_bound.set_metric_value(
                            metric_name, metric_avg_value
                        )
                    )
                    constant_memory_bound_description_has_found = (
                        self.__memory_constant_memory_bound.set_metric_description(
                            metric_name, metric_description
                        )
                    )
                    if (
                        not constant_memory_bound_value_has_found
                        or not constant_memory_bound_description_has_found
                    ):
