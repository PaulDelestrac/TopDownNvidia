...
class LevelThreeNsight(LevelThree, LevelTwoNsight):
    ...
    def __init__(
        ...
        front_decode: FrontDecodeNsight,
        front_fetch: FrontFetchNsight,
        back_core_bound: BackCoreBoundNsight,
    ...
    def _generate_command(self) -> str:
        ...
        command: str = (
            ...
            + ","
            + self.__memory_mio_throttle.metrics_str()
            + ","
            + self.__memory_l1_bound.metrics_str()
            + " "
            + self._program
        )
        return command
    ...
    def set_results(self, output_command: str):
        ...
        super()._set_front_back_divergence_retire_results(
            output_command
        )  # level one results
        super()._set_memory_core_decode_fetch_results(output_command)  # level two
        self._set_memory_constant_memory_bound_mio_l1_bound_results(
            output_command
        )  # level three
    ...
    def _get_results(self, lst_output: list):
        ...
        if self._collect_metrics and self.__memory_mio_throttle.metrics_str() != "":
            lst_output.append(
                converter.underlined_str(self.__memory_mio_throttle.name())
            )
            super()._add_result_part_to_lst(
                self.__memory_mio_throttle.metrics(),
                self.__memory_mio_throttle.metrics_description(),
                lst_output,
            )
        if self._collect_metrics and self.__memory_l1_bound.metrics_str() != "":
            ...
            super()._add_result_part_to_lst(
                self.__memory_l1_bound.metrics(),
                self.__memory_l1_bound.metrics_description(),
                lst_output,
            )
        ...
        if self._collect_metrics and self._retire.metrics_str() != "":
            ...
            super()._add_result_part_to_lst(
                self._retire.metrics(), self._retire.metrics_description(), lst_output
            )
    ...
    def _metricExists(self, metric_name: str) -> bool:
        ...
        is_defined_front_fetch_unit: str = (
            super().front_fetch().get_metric_unit(metric_name)
        )

        is_defined_front_decode_value: str = (
            super().front_decode().get_metric_value(metric_name)
        )
        is_defined_front_decode_unit: str = (
            super().front_decode().get_metric_unit(metric_name)
        )

        is_defined_back_memory_bound_value: str = (
            super().back_memory_bound().get_metric_value(metric_name)
        )
        is_defined_back_memory_bound_unit: str = (
            super().back_memory_bound().get_metric_unit(metric_name)
        )

        is_defined_back_core_bound_value: str = (
            super().back_core_bound().get_metric_value(metric_name)
        )
        is_defined_back_core_bound_unit: str = (
            super().back_core_bound().get_metric_unit(metric_name)
        )

        if not (
            (
                is_defined_front_fetch_value is None
                and is_defined_front_fetch_unit is None
            )
            or (
                is_defined_front_decode_value is None
                and is_defined_front_decode_unit is None
            )
            or (
                is_defined_back_memory_bound_value is None
                and is_defined_back_memory_bound_unit is None
            )
            or (
                is_defined_back_core_bound_value is None
                or is_defined_back_core_bound_unit is None
            )
        ):
            return False
        return True

    def _set_memory_constant_memory_bound_mio_l1_bound_results(
        self, results_launch: str
    ):
        """
        ...
        for line in str(results_launch).splitlines():
            ...
            if not can_read_results:
                ...
                continue
            if (len(list_words) == 4 or len(list_words) == 3) and list_words[1][
                0
            ] != "-":
                if len(list_words) == 3:
                ...
                memory_constant_memory_bound_value_has_found = (
                    self.__memory_constant_memory_bound.set_metric_value(
                        metric_name, metric_value
                    )
                )
                memory_constant_memory_bound_unit_has_found = (
                    self.__memory_constant_memory_bound.set_metric_unit(
                        metric_name, metric_unit
                    )
                )
                memory_mio_throttle_value_has_found = (
                    self.__memory_mio_throttle.set_metric_value(
                        metric_name, metric_value
                    )
                )
                memory_mio_throttle_unit_has_found = (
                    self.__memory_mio_throttle.set_metric_unit(metric_name, metric_unit)
                )
                memory_l1_bound_value_has_found = (
                    self.__memory_l1_bound.set_metric_value(metric_name, metric_value)
                )
                memory_l1_bound_unit_has_found = self.__memory_l1_bound.set_metric_unit(
                    metric_name, metric_unit
                )
                if not (
                    memory_constant_memory_bound_value_has_found
                    or memory_mio_throttle_value_has_found
                    or memory_l1_bound_value_has_found
                ) or not (
                    memory_constant_memory_bound_unit_has_found
                    or memory_mio_throttle_unit_has_found
                    or memory_l1_bound_unit_has_found
                ):
    ...
    def memory_mio_throttle_stall_on_memory_bound(self) -> float:
        ...
        return (
            self.memory_mio_throttle_stall() / super().back_memory_bound_stall()
        ) * 100.0
    ...
    def memory_mio_throttle_percentage_ipc_degradation(self) -> float:
        ...
        return (
            (self._stall_ipc() * (self.memory_mio_throttle_stall() / 100.0))
            / self.get_device_max_ipc()
        ) * 100.0
    ...
    def memory_l1_bound_stall_on_memory_bound(self) -> float:
        ...
        return (
            self.memory_l1_bound_stall() / super().back_memory_bound_stall()
        ) * 100.0
    ...
    def memory_l1_bound_percentage_ipc_degradation(self) -> float:
        ...
        return (
            (self._stall_ipc() * (self.memory_l1_bound_stall() / 100.0))
            / self.get_device_max_ipc()
        ) * 100.0
