...
class LevelTwoNsight(LevelTwo, LevelOneNsight):
    ...
    def __init__(
        ...
        self._branch_divergence: DivergenceBranchNsight = DivergenceBranchNsight(
            DivergenceBranchParameters.C_DIVERGENCE_BRANCH_NAME,
            DivergenceBranchParameters.C_DIVERGENCE_BRANCH_DESCRIPTION,
            "",
        )
        self._replay_divergence: DivergenceReplayNsight = DivergenceReplayNsight(
    ...
    def _branch_divergence_ipc_degradation(self) -> float:
        ...
        return super()._branch_diver_ipc_degradation(
            LevelExecutionParameters.C_WARP_EXECUTION_EFFICIENCY_METRIC_NAME_NSIGHT
        )
    ...
    def _metricExists(self, metric_name: str) -> bool:
        ...
        is_defined_front_end_unit: str = (
            super().front_end().get_metric_unit(metric_name)
        )
        ...
        is_defined_divergence_unit: str = (
            super().divergence().get_metric_unit(metric_name)
        )
        ...
        is_defined_extra_measure_unit: str = (
            super().extra_measure().get_metric_unit(metric_name)
        )

        if not (
            (is_defined_front_end_value is None and is_defined_front_end_unit is None)
            or (is_defined_back_end_value is None and is_defined_back_end_unit is None)
            or (
                is_defined_divergence_value is None
                and is_defined_divergence_unit is None
            )
            or (is_defined_retire_value is None or is_defined_retire_unit is None)
            or (
                is_defined_extra_measure_value is None
                and is_defined_extra_measure_unit is None
            )
        ):
            return False
        return True
        ...
        for line in str(results_launch).splitlines():
            ...
            if (len(list_words) == 4 or len(list_words) == 3) and list_words[1][
                ...
                back_core_bound_value_has_found = (
                    self._back_core_bound.set_metric_value(metric_name, metric_value)
                )
                back_core_bound_unit_has_found = self._back_core_bound.set_metric_unit(
                    metric_name, metric_unit
                )
                back_memory_bound_value_has_found = (
                    self._back_memory_bound.set_metric_value(metric_name, metric_value)
                )
                back_memory_bound_unit_has_found = (
                    self._back_memory_bound.set_metric_unit(metric_name, metric_unit)
                )
                front_decode_value_has_found = self._front_decode.set_metric_value(
                    metric_name, metric_value
                )
                front_decode_unit_has_found = self._front_decode.set_metric_unit(
                    metric_name, metric_unit
                )
                front_fetch_value_has_found = self._front_fetch.set_metric_value(
                    metric_name, metric_value
                )
                front_fetch_unit_has_found = self._front_fetch.set_metric_unit(
                    metric_name, metric_unit
                )
                if not (
                    back_core_bound_value_has_found
                    ...
                    back_core_bound_unit_has_found
                    or back_memory_bound_unit_has_found
                    or front_decode_unit_has_found
                    or front_fetch_unit_has_found
                ):
